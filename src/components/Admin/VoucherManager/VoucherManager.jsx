...
const VoucherManager = () => {
  ...
  const handleDelete = (id) => { deleteVoucher(id); load(); };
  const handleToggle = (v) => { updateVoucher(v.id, { active: !v.active }); load(); };
          ...
          <TableBody>
            {vouchers.map((v, i) => (
              <TableRow key={v.id} hover>
                ...
                <TableCell>{exams.find((e) => e.id === v.examId)?.name || "N/A"}</TableCell>
                <TableCell>
        ...
        <DialogContent sx={{ pt: "16px !important" }}>
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select value={form.examId} label="Exam" onChange={(e) => setForm({ ...form, examId: e.target.value })}>
              {exams.map((e) => <MenuItem key={e.id} value={e.id.toString()}>{e.name}</MenuItem>)}
            </Select>
          </FormControl>
        </DialogContent>
        <DialogActions>
