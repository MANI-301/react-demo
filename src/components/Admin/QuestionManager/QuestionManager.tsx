...
const QuestionManager = () => {
  ...
  const loadQuestions = (eid: number) => setQuestions(getQuestionsByExam(eid));

  useEffect(() => { loadExams(); }, []);
  useEffect(() => { if (selectedExam) loadQuestions(selectedExam.id); }, [selectedExam]);

  const openExamDialog = (e?: any) => {
  ...
  const removeExam = (id: number) => { deleteExam(id); loadExams(); if (selectedExam?.id === id) { setSelectedExam(null); setQuestions([]); } };

  const openQDialog = (q?: any) => {
  ...
  const removeQ = (id: number) => { deleteQuestion(id); if (selectedExam) loadQuestions(selectedExam.id); };
